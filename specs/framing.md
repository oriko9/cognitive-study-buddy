...
| **The LLM provider (OpenRouter, `openai/gpt-oss-20b`, paid)** | Runtime dependency, metered per token | Bounded call volume; the app must degrade honestly when the account's credit runs out, not loop |
...
| N8 | The model is pinned to the explicit id `openai/gpt-oss-20b` (paid; chosen over the single-provider `openai/gpt-5-nano` for its 12-provider failover on OpenRouter and lower cost — see Turn 6). No moving alias appears anywhere in the codebase. | Grep gate in CI (`check:pins`) |
...
| O2 | Model id and quota/cost | **Resolved 2026-09-23 (Turn 6), supersedes Turn 5.** Moved from `meta-llama/llama-3.1-8b-instruct` to `openai/gpt-oss-20b`: cheaper (~$0.018/M input, $0.09/M output vs. ~$0.02/$0.04 — note output is pricier per token, but Turn 6 also capped `max_tokens` per call site, so the effective per-cycle cost is lower, not higher) and open-weight, served by 12 providers on OpenRouter rather than one — a reliability property, not just a price one, chosen after this project hit two single-source failures in a row (Gemini's rate limit, then the llama `:free` 404). There is no fixed RPD/RPM quota on a metered paid model — the operative ceiling is a **per-key credit limit set on the OpenRouter account itself** (outside this repo), not a rate. See N11 for what that means for the client-side counter. |

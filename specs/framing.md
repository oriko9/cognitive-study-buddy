...
| **The LLM provider (OpenRouter, `meta-llama/llama-3.1-8b-instruct:free`)** | Runtime dependency, free tier | Bounded call volume; the app must degrade honestly when quota is exhausted, not loop |
...
| N8 | The model is pinned to the explicit id `meta-llama/llama-3.1-8b-instruct:free`. No moving alias appears anywhere in the codebase. | Grep gate in CI (`check:pins`) |
...
| O2 | Model id and free-tier quota | **Reopened 2026-09-22.** The provider changed from Gemini to OpenRouter (`meta-llama/llama-3.1-8b-instruct:free`) — see Turn 4. The prior resolution (`models/gemini-3.1-flash-lite`, 15 RPM / 250K TPM / 500 RPD) no longer applies; those numbers were specific to Gemini's own AI Studio dashboard. OpenRouter's actual rate/token/daily limits for this model have not yet been measured against the account's own dashboard — **N11's client-side cap should not be treated as validated against the real ceiling until that measurement is taken.** |

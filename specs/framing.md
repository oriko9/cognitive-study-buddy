...
| **The LLM provider (OpenRouter, `meta-llama/llama-3.1-8b-instruct`, paid)** | Runtime dependency, metered per token | Bounded call volume; the app must degrade honestly when the account's credit runs out, not loop |
...
| N8 | The model is pinned to the explicit id `meta-llama/llama-3.1-8b-instruct` (paid — the `:free` variant returned 404 in production, per Turn 4/5). No moving alias appears anywhere in the codebase. | Grep gate in CI (`check:pins`) |
...
| O2 | Model id and quota/cost | **Resolved 2026-09-23 (Turn 5), supersedes the 2026-09-22 reopening.** The `:free` OpenRouter variant returned a 404 ("unavailable for free") in production — free tier was never actually usable. Moved to the paid `meta-llama/llama-3.1-8b-instruct`, ~$0.02/M input and $0.04/M output tokens at time of writing (OpenRouter's own pricing page, not assumed). There is no fixed RPD/RPM quota to measure on a metered paid model — the operative ceiling is a **per-key credit limit set on the OpenRouter account itself** (outside this repo), not a rate. See N11 for what that means for the client-side counter. |

...
| **The LLM provider (OpenRouter, `openai/gpt-5-nano`, paid)** | Runtime dependency, metered per token, single-source (OpenAI only) | Bounded call volume; the app must degrade honestly when the account's credit runs out, not loop |
...
| # | Criterion | How it is verified |
| --- | --- | --- |
| N1 | The OpenRouter API key exists **only** as a server-side Vercel environment variable (`OPENROUTER_API_KEY`, no `VITE_` prefix). No key, and no direct provider call, appears in any client bundle. | `npm run check:secrets` scans **two surfaces**: the built output in `dist/`, for known key formats — legacy Gemini `AIza`, current Gemini `AQ.`, and OpenRouter's `sk-or-v1-` — plus the literal `GEMINI_API_KEY` and `OPENROUTER_API_KEY`, since either variable name alone in a bundle means Vite inlined it; and **every git-tracked file**, for the same key formats plus either variable name assigned a non-empty value, since the names appear legitimately in source and specs but an assigned value never should. No allowlist. Fails on a match in either surface |
...
| N8 | The model is pinned to the explicit id `openai/gpt-5-nano` (paid; chosen for latency over the 12-provider `openai/gpt-oss-20b` — see Turn 7). GPT-5-family models reject a non-default `temperature` and the `max_tokens` key, requiring `max_completion_tokens` instead; `call-model.ts` branches on the `openai/gpt-5` prefix to send the shape each family accepts. No moving alias appears anywhere in the codebase. | Grep gate in CI (`check:pins`); adapter request-builder tests cover both parameter shapes |
| N9 | The provider key is sent in the `Authorization: Bearer` **header**. No key-bearing query parameter appears anywhere. | Unit test on the adapter's request builder |
| N11 | A **client-side** rate limit: a per-browser counter in `localStorage` refuses a new cycle after **10 in a rolling 24 hours**, rendered as a named "daily limit reached" state — never a blank screen or a generic error. What it protects: an ordinary user from spending the shared quota by accident, in the case that actually happens. What it does not protect: anything, against anyone who clears site data or opens a private window, either of which resets the counter in one action. There is no server to enforce it on and no identity to enforce it against, so this is a courtesy to the honest user, not a control. **Since Turn 5, the provider is a paid, metered model (N8) — bypassing this counter no longer "costs nothing"; it costs real money on the deployment owner's OpenRouter account.** The only actual backstop against sustained abuse is a per-key credit limit set on the OpenRouter account itself, outside this codebase and outside this criterion's check. This criterion does not raise, and cannot enforce, that ceiling. | Unit test on the counter module |
...
| O2 | Model id and quota/cost | **Resolved 2026-09-23 (Turn 7), supersedes Turn 6.** Moved from `openai/gpt-oss-20b` to `openai/gpt-5-nano` for latency. Costs more per token (~$0.05/M input, $0.40/M output vs. ~$0.018/$0.09) and trades away gpt-oss-20b's 12-provider OpenRouter failover for a single proprietary source (OpenAI) — accepted knowingly for the speed gain, not unnoticed. Uncapped by anything in this repo: there is no fixed RPD/RPM quota on a metered paid model, the operative ceiling is a **per-key credit limit set on the OpenRouter account itself** (outside this repo), not a rate. See N11 for what that means for the client-side counter. **Not yet measured:** whether gpt-5-nano is actually faster than gpt-oss-20b in this app's own request pattern — the latency claim motivating this switch has not been benchmarked here, only asserted. |
...
| O5 | Whether the pinned model grades free-text answers reliably enough | **Reopened 2026-09-22.** Was resolved 2026-09-08 for `gemini-3.1-flash-lite` (evidence kept below, unchanged, as historical record) — but Turn 4 switched the provider to OpenRouter's `meta-llama/llama-3.1-8b-instruct:free`, and that evidence does not transfer to a different model. Grading reliability for the new model is **untested**; the Turn 2 fixture (`src/fixtures/o5-answers-he.md`) has not yet been re-run against it. Do not treat grading as validated until it is. **2026-09-08 evidence for the retired Gemini pin:** five hand-labelled Hebrew answers run by hand through the live deploy on one real question — correct scored 1.0, partial 0.7, and the confidently-wrong, off-topic and empty cases all scored 0. The key case — an answer reusing the correct answer's own formula but drawing the opposite false conclusion — scored 0, evidence the model graded meaning rather than pattern-matching formula presence. Stated honestly then and still true: this was one question with one answer per label — evidence, not proof. |

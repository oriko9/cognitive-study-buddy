...
## Turn 4 — Provider pivot under production failure (opened 2026-09-22)

- **Module beat:** M9 (economics) · M12 (safety and trajectory)
- **Trigger, not a plan.** Unlike Turns 2 and 3, this turn was not scoped in
  advance. Ori reported the live Vercel deployment returning **502s** to real
  users, attributed to the Gemini free-tier rate limit (O2: 15 RPM / 500 RPD)
  being exhausted under real usage. That report is recorded as Ori's
  observation — this session did not independently inspect Vercel logs to
  confirm the 502s or their cause before the fix landed.
- **Decision:** switch provider from Gemini to OpenRouter, model pinned to
  `meta-llama/llama-3.1-8b-instruct:free`, to get out from under the
  exhausted Gemini quota. Ori updated `GEMINI_API_KEY` → `OPENROUTER_API_KEY`
  by hand in `.env.local` and in the Vercel project settings; that manual
  step is outside this repository's audit trail by nature (an env var is not
  a diff) and is recorded here instead.
- **What changed in code (already in the working tree when this turn
  started, reviewed here rather than authored here):** `api/_lib/call-model.ts`
  — new endpoint (`openrouter.ai/api/v1/chat/completions`), OpenAI-shaped
  request/response, key moved from the `x-goog-api-key` header to
  `Authorization: Bearer`. `api/_lib/env.ts` — `SERVER_KEY` renamed to
  `OPENROUTER_API_KEY`. `scripts/check-secrets.mjs` — extended to also scan
  for the `sk-or-v1-` key format and the `OPENROUTER_API_KEY` name, alongside
  (not instead of) the existing Gemini patterns. Adapter and handler tests
  updated to match.
- **Verified this session:** `npm run verify` (93 tests, all green),
  `npm run check:pins`, and `npm run check:secrets` all pass against the new
  code. This confirms the switch is internally consistent and the gates
  still hold — it does not confirm the original 502s are gone in production,
  which only continued live use can show.
- **Changed as a result, in `specs/framing.md`:** §2 stakeholder row, N1
  (env var name), N8 (model id), N9 (header form) updated to match the code.
  **O2 reopened** — the Gemini quota numbers it recorded no longer apply, and
  OpenRouter's actual rate/token/daily limits for this model have not been
  measured yet, so N11's client-side cap is unvalidated against the real
  ceiling until that measurement happens. **O5 reopened** — its resolution
  was evidence specific to `gemini-3.1-flash-lite`'s grading behaviour; that
  evidence is kept as historical record but does not transfer to the new
  model, whose grading reliability is untested. Re-running the Turn 2 fixture
  (`src/fixtures/o5-answers-he.md`) against the new model is the obvious next
  turn.
- **Known gap, named rather than hidden:** `CLAUDE.md` §1 (stack line, pinned
  model) and `specs/specification.md` §3.5 (adapter contract) still describe
  the retired Gemini contract. `call-model.ts`'s own header comment already
  flags this. Left untouched in this turn by scope decision, not oversight —
  carried forward for whoever picks it up next.
- **Commit range:** not yet assigned. The code and env changes described
  above were already in the working tree, uncommitted, when this entry was
  written; per the convention Turn 0 set, the range is recorded when the
  turn actually closes with a commit, not invented ahead of it.

**Turn 4 is open.** It differs from every prior turn in kind: Turns 1–3 were
planned observations against a design; this one is an unplanned pivot forced
by a real deployment failing under real load — the free-tier economics (M9)
this project accepted in Turn 0 turning out to bind sooner than assumed. It
reopens two decisions (O2, O5) rather than quietly overwriting them, because
resolving a decision by evidence and then swapping out what the evidence was
about without saying so would make the trail dishonest. Open going forward:
measuring OpenRouter's real limits, re-validating grading reliability against
the new model, and the CLAUDE.md/specification.md gap named above.
